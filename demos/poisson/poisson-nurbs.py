...
from tIGAr.common import EqualOrderSpline, ExtractedSpline, mpirank, worldcomm
...
for level in range(0, N_LEVELS):
    ...
    u = spline.rationalize(dolfin.TrialFunction(spline.V))

    # Corresponding test function.
    v = spline.rationalize(dolfin.TestFunction(spline.V))
    ...
    u_hom = dolfin.Function(spline.V)
    ...
    u_hom.rename("u", "u")
    dolfin.File("results/u.pvd") << u_hom
    ...
    for i in range(0, nsd + 1):
        ...
        dolfin.File("results/" + name + "-file.pvd") << spline.cpFuncs[i]
    ...
    L2_error = np.sqrt(
        dolfin.assemble(((spline.rationalize(u_hom) - soln) ** 2) * spline.dx)
    )
