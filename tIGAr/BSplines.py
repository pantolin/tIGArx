...
#                                               cppargs='-g -O2')
basisFuncsCXXModule = dolfin.jit.pybind11jit.compile_cpp_code(basisFuncsCXXString)
...
class BSpline1(object):
    ...
    def __init__(self, p, knots):
        ...
        for i in range(0, len(self.knots)):
            if lastKnot == None or (
                not dolfin.near(self.knots[i], lastKnot, eps=KNOT_NEAR_EPS)
            ):
                ct += 1
    ...
    def computeNel(self):
        ...
        for i in range(1, len(self.knots)):
            if not dolfin.near(self.knots[i], self.knots[i - 1], eps=KNOT_NEAR_EPS):
                self.nel += 1
    ...
    def basisFuncs(self, knotSpan, u):
        ...
        basisFuncsInner(self.ghostKnots, self.nGhost, u, pl, i, ndu, left, right, ders)
...
class BSpline(AbstractScalarBasis):
    ...
    def __init__(self, degrees, kvecs, useRect=USE_RECT_ELEM_DEFAULT, overRefine=0):
        """
    ...
    def generateMesh(self, comm=worldcomm):
        if self.nvar == 1:
            ...
            mesh = dolfin.UnitSquareMesh.create(
            ...
            mesh = dolfin.UnitCubeMesh.create(
...
class MultiBSpline(AbstractScalarBasis):
    ...
    def generateMesh(self, comm=worldcomm):
        ...
        comm.Barrier()
        mesh = dolfin.Mesh(comm, MESH_FILE_NAME)
...
class ExplicitBSplineControlMesh(AbstractControlMesh):
    ...
    def __init__(
        self, degrees, kvecs, extraDim=0, useRect=USE_RECT_ELEM_DEFAULT, overRefine=0
    ):
        """
        ...
        self.scalarSpline = BSpline(
            degrees, kvecs, useRect=useRect, overRefine=overRefine
        )
        # parametric == physical
        self.nvar = len(degrees)
    ...
    def getHomogeneousCoordinate(self, node, direction):
        ...
        if direction < self.nvar:
            ...
            coord = self.scalarSpline.splines[direction].greville(directionalIndex)
        else:
            coord = 0.0
        return coord
...
class LegacyMultipatchControlMesh(AbstractControlMesh):
    ...
    def __init__(
        self, prefix, nPatch, suffix, useRect=USE_RECT_ELEM_DEFAULT, overRefine=0
    ):
        """
        ...
        for i in range(0, nPatch):
            ...
            splines += [
                BSpline(degrees, kvecs, useRect, overRefine),
            ]
