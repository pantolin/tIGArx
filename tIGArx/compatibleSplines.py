...
class BSplineCompat(AbstractMultiFieldSpline):
    ...
    def customSetup(self, args):
        ...
        self.fields = generateFieldsCompat(
            self.controlMesh, self.RTorN, self.degrees, periodicities=self.periodicities
        )
...
def divFreeProject(
    ...
    if getOtherFields != None:
        ...
        res += ufl.inner(p, q) * spline.dx
    iteratedDivFreeSolve(
        res,
        u_hat,
        v_hat,
        spline,
        divOp=lambda up: ufl.div(getVelocity(up)),
        penalty=penalty,
        w=w,
        applyBCs=applyBCs,
    )
...
class ExtractedBSplineRT(ExtractedSpline):
    ...
    def iteratedDivFreeSolve(
        self, residualForm, u, v, penalty=DEFAULT_RT_PENALTY, w=None, applyBCs=True
    ):
        """
        ...
        """
        iteratedDivFreeSolve(
            residualForm, u, v, self, penalty=penalty, w=w, applyBCs=applyBCs
        )

    def divFreeProject(
        self, toProject, penalty=DEFAULT_RT_PENALTY, w=None, applyBCs=True
    ):
        """
        Wrapper for free function ``divFreeProject``, largely included
        for backward compatibility.
        """
        return divFreeProject(toProject, self, penalty=penalty, w=w, applyBCs=applyBCs)
