...
class TimingLogger:
    ...
    def start_timing(self, label, print=False):
        if mpirank == 0:
            ...
            self.logs.append((label, self.current_level))
            self.start_times.append((label, MPI.Wtime()))
            self.current_level += 1

            if print:
                self.print_pending(self.logs[-1])

    def end_timing(self, label, print=True):
        if mpirank == 0:
            end_time = MPI.Wtime()
            _label, start_time = self.start_times.pop()

            if _label != label:
                raise ValueError(f"Timing for '{_label}' was not started.")

            elapsed_time = end_time - start_time

            for i in range(len(self.logs) - 1, -1, -1):
                if len(self.logs[i]) == 3:
                    continue

                log_label, level = self.logs[i]

                if log_label == label and level == self.current_level - 1:
                    self.logs[i] = (log_label, level, elapsed_time)
                    if print:
                        self.print_completed(self.logs[i])
                    break

            self.current_level -= 1

    @staticmethod
    def print_pending(log):
        label, current_level = log
        indent = " " * (current_level * 4)
        print(f"{indent}{label}...")

    @staticmethod
    def print_completed(log):
        label, current_level, elapsed_time = log
        indent = " " * (current_level * 4)
    ...
    def print_log(self):
        if mpirank == 0:
            for log in self.logs:
                if len(log) == 2:  # Log entry with timing information
                    self.print_completed(log)
                else:  # Log entry without timing information
                    self.print_pending(log)
